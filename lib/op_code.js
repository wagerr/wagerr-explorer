...
const buildTypes = {
    ...
    peerlessParlayBet: ['prefix', 'version', 'txType', 'legCounts'],
...
const peerlessParlayBet = {
  prefix: buildProperty('String'),
  version: buildProperty('Decimal', 2),
  txType: buildProperty('Decimal', 2),
  legCounts: buildProperty('Decimal', 2),
};
...
async function getMappingName(mappingIndex, id) {
  let response;
  //console.log('getMappingName', mappingIndex, id);
  try {
    response = await rpc.call('getmappingname', [mappingIndex, id]);
    //console.log('getMappingName', response);
  } catch (e) {
    console.log('error', e);
    log(e);
...
function bufferDecrypt(hex) {
  return Buffer.from(hex, 'hex').readIntLE(0, 1);
}
...
function decryptFromLittleEndian(littleEn, format="INT32") {
  ...
  let decryptedVal = parseInt(hex, 16)

  if (format == "INT16"){
    if ((decryptedVal & 0x8000) > 0) {
      decryptedVal = decryptedVal - 0x10000;
   }
  }
  return decryptedVal;
}
...
async function handleSingleMappingRequest(id, fieldData, opcode) {
  ...
  try {
    //console.log('handleSingleMappingRequest-inner query start', decryptedSection, fieldData.mappedData);
    const mappingname_item = await Mappingname.findOne({
      mappingId: decryptedSection,
      mappingIndex: fieldData.mappedData,
    }).sort({mappingId: -1});
    console.log('mappingname_item', mappingname_item);
    //console.log('handleSingleMappingRequest-inner query end mappingname_item', mappingname_item);

    if (!mappingname_item) {
      mappingNameData = await getMappingName(fieldData.mappedData, decryptedSection);      
      try {
        console.log('mappingNameData', fieldData.mappedData, decryptedSection, mappingNameData)
      ...
      mappingNameData = {
        'mapping-name': mappingname_item.name,
        'mapping-index': mappingname_item.mappingIndex,
        'mapping-id': mappingname_item.mappingId,
      };
    }
  } catch (e) {
    console.log('error', e);
...
async function decode(hexValue, decodeType = 'hex', showLogs = false) {
  ...
  for (let x = 0; x < fieldsToFill.length; x +=1) {
    ...
    if (transaction.txType == 'peerlessParlayBet'){      
      let legCounts = parseInt(hexValue.substr(currentPos, slices));
      console.log('hexValue, legCounts', hexValue, legCounts);
      
      transaction[f] = legCounts;
      transaction.opCode = hexValue;      

      currentPos += slices;

      let betdata = [];      
      for (let legindex = 0; legindex < legCounts; legindex++){
        let decryptedValue = 'unknown';
        try {
          let eventIdSection = hexValue.substr(currentPos, 8);
          let newWay = decryptFromLittleEndian(eventIdSection);
          decryptedValue = newWay;
        } catch (e) {
          log(e);          
        }
        let eventId = decryptedValue;
        decryptedValue = 'unknown';
        currentPos += 8;        
        try {
          let outcomeSection = hexValue.substr(currentPos, 2);
          let newWay = decryptFromLittleEndian(outcomeSection);
          decryptedValue = newWay;
        } catch (e) {
          log(e);          
        }
        let outcome = decryptedValue;
        currentPos += 2;        
        betdata.push({eventId: eventId, outcome: outcome});
      }

      transaction.betdata = betdata;
    ...
    if ((dataType === 'Decimal' || dataType === 'LittleEndian') && fieldData.mappedData) {
      ...
      try {
        let newWay = decryptFromLittleEndian(thisSection);
        if (f == "spreadPoints"){
          newWay = decryptFromLittleEndian(thisSection, 'INT16');
        }         
        decryptedValue = newWay;
      } catch (e) {
        log(e);
