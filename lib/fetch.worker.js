...
const crosschainApi = `${config.crosschain_backend.api}`;

// Get the address and all transactions related.
const getAddress = ({ address, ...query }) =>
  fetch(`${api}/address/${address}`, query);
...
const getCoins = async (query) => {
  try {
    ...
    return Promise.resolve(
      coins.map((c) => ({ ...c, avgBlockTime, avgMNTime }))
    );
  } catch (err) {
...
const getListEvents = (query) =>
  fetch(`${crosschainApi}/listevents`, query);
...
const getBetUpdates = (query) => fetch(`${api}/bet/updates?eventId=${query}`);

const getBetEventInfo = (query) => fetch(`${api}/bet/event/${query}/info`);

const getBetEventsInfo = (query) => fetch(`${api}/bet/events/info`, query);

const getBetParlaysInfo = (query) => fetch(`${api}/bet/parlaybets`, query);

const getBetsForAccount = (query) =>
  fetch(`${api}/custom/getbetsforaccount/${query}`);

const getCurrentPps = (query) => fetch(`${api}/pps/current`, query);

const getBetPerWeek = (query) => fetch(`${api}/statistic/perweek`, query);

const getBettingStatData = (query) => fetch(`${api}/stat/betting`, query);
const getMasternodeStatData = (query) => fetch(`${api}/stat/masternode`, query);
...
const getLottoResults = (query) =>
  fetch(`${api}/lotto/results?eventId=${query}`);

const getLottoEventInfo = (query) => fetch(`${api}/lotto/event/${query}/info`);

const getCrosschainBetsByAccount = (query) =>
  fetch(`${crosschainApi}/bet/${query.chain}/betsByAccount/${query.account}`);

const getCrosschainBetByTxId = (query) =>
  fetch(`${crosschainApi}/bet/${query.chain}/betByTxId/${query.txid}`);
...
self.addEventListener("message", (ev) => {
  ...
  switch (ev.data.type) {
    ...
    case "beteventsinfo":
      ...
      break;
    case "betparlaysinfo":
      action = getBetParlaysInfo;
      break;
    case "getbetsforaccount":
      action = getBetsForAccount;
      break;
    case "currentpps":
    ...
    case "betperweek":
      ...
      break;
    case "betstatdata":
      action = getBettingStatData;
      break;
    case "masternodestatdata":
      action = getMasternodeStatData;
      break;
    case "opcode":
      action = getOpCode;
      break;
    ...
    case "lottoeventinfo":
      ...
      break;
    case "crosschainBetsByAccount":
      action = getCrosschainBetsByAccount;
      break;
    case "crosschainBetByTxId":
      action = getCrosschainBetByTxId;
  }
