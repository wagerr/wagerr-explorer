...
class EventList extends Component {
  ...
  filterEvents = () => {
    ...
    if (sport == "allevent") {
      ...
      const filtered = this.state.events.filter(
        (e) => e.sport.toLowerCase() == sport
      );
    ...
    if (this.state.search !== "") {
      const searchedEvents = tempevents.filter((e) => {
        ...
        return (
          e.tournament.toLowerCase().includes(searchValue) ||
    ...
    return (
      <div>
        ...
        <div className="scrollable">
          {" "}
          {this.state.filteredEvents.map((event, key) => {
            if (
              !event.odds[0].mlHome &&
              !event.odds[0].mlAway &&
              !event.odds[0].mlDraw &&
            ...
            let mlHomeEO = ClientUtils.convertToOdds(
              moneylineHomeOdds,
              false,
              true
            );
            let mlAwayEO = ClientUtils.convertToOdds(
              moneylineAwayOdds,
              false,
              true
            );
            let mlDrawEO = ClientUtils.convertToOdds(
              moneylineDrawOdds,
              false,
              true
            );

            moneylineHomeOdds = ClientUtils.convertToOdds(
              moneylineHomeOdds,
              toggleSwitchOddsStyle,
              toggleSwitchOdds
            );
            moneylineAwayOdds = ClientUtils.convertToOdds(
              moneylineAwayOdds,
              toggleSwitchOddsStyle,
              toggleSwitchOdds
            );
            moneylineDrawOdds = ClientUtils.convertToOdds(
              moneylineDrawOdds,
              toggleSwitchOddsStyle,
              toggleSwitchOdds
            );
            ...
            let spreadHomeEO = ClientUtils.convertToOdds(
              spreadHomeOdds,
              false,
              true
            );
            let spreadAwayEO = ClientUtils.convertToOdds(
              spreadAwayOdds,
              false,
              true
            );

            spreadHomeOdds = ClientUtils.convertToOdds(
              spreadHomeOdds,
              toggleSwitchOddsStyle,
              toggleSwitchOdds
            );
            spreadAwayOdds = ClientUtils.convertToOdds(
              spreadAwayOdds,
              toggleSwitchOddsStyle,
              toggleSwitchOdds
            );
            ...
            let totalsOverEO = ClientUtils.convertToOdds(
              totalsOverOdds,
              false,
              true
            );
            let totalsUnderEO = ClientUtils.convertToOdds(
              totalsUnderOdds,
              false,
              true
            );

            totalsOverOdds = ClientUtils.convertToOdds(
              totalsOverOdds,
              toggleSwitchOddsStyle,
              toggleSwitchOdds
            );
            totalsUnderOdds = ClientUtils.convertToOdds(
              totalsUnderOdds,
              toggleSwitchOddsStyle,
              toggleSwitchOdds
            );
            ...
            tempevent.disabled =
              this.disabledEvents.includes(tempevent.event_id) &&
              this.betType === "parlay";

            return <CardBettingEvent data={tempevent} key={key} />;
          })}
        </div>
      </div>
    );
